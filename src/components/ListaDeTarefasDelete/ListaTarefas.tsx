...
function ListaTarefas({ lista }: ListaTarefasProps) {
  ...
  function adicionaTarefa() {
    const id =
      listaDeTarefas
        ...
        .reduce((max, val) => Math.max(max, val), -Infinity) + 1;
  ...
  function toggleCompleta(id: number) {
    setListaDeTarefas(
  ...
  function eliminaItem(id: number) {
      ...
      <ul>
        {listaFiltrada.map((tarefa) => (
          <ItemTarefa
            key={tarefa.id}
            tarefa={tarefa}
            eliminaTarefa={eliminaItem}
            toggleCompleta={toggleCompleta} // Passa a função para o componente filho
          />
        ))}
      </ul>
    </div>
